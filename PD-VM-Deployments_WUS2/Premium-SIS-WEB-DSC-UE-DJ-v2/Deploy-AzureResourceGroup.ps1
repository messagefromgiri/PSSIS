...
Param(
    ...
    [string] $SubscriptionName='PS-EXT-PD-SNGL-USW',
    [string] $ResourceLocation='West US 2',
    [string] $ResourceGroupName = 'SIS-PD-ST-20-FE-RG001',
    [switch] $UploadArtifacts = $true,
    [string] $StorageAccountName = "sispd20sragrssa003",
...
foreach($AzureVMName in $AzureVMNames){
                ...
                $OptionalParameters['virtualNetworkResourceGroup'] = 'TIER-PD-NW-20-T0-RG001'
                $OptionalParameters['virtualNetworkName'] = 'PS-EXT-PD-SNGL-20-VN001'
                $OptionalParameters['subnetName'] = "SIS-WB-SN001-172.21.128.0_21"
                $OptionalParameters['domainToJoin'] = "powerschool.host"
                $OptionalParameters['domainUsername'] = "sundeep.paluru"
	                ##Golden Image Value
                $OptionalParameters['templateResourceID'] = '/subscriptions/fedb6b9b-8def-4ccb-947b-4a87809a7b2e/resourceGroups/SIS-PD-IM-20-T0-RG001/providers/Microsoft.Compute/images/SISPDWB20WIMG001'
	                ##Key Vault
                ...
                $OptionalParameters['pIPAddress'] = Powershell -file ..\..\Scripts\Set-IPAllocator.ps1 -Path ..\..\Scripts\SIS-WB-SN001-172.21.128.0_21.csv -HostName ($AzureVMName+"-Nic")
                $OptionalParameters['sIPAddress'] = Powershell -file ..\..\Scripts\Set-IPAllocator.ps1 -Path ..\..\Scripts\SIS-WB-SN001-172.21.128.0_21.csv -HostName ($AzureVMName+"-BNic")
