...
Param(
    ...
    [string] $ResourceGroupName = 'SIS-PD-ST-02-FE-RG001',
    [switch] $UploadArtifacts = $true,
    [string] $StorageAccountName = "sispd02sragrssa003",
...
foreach ($AzureVMName in $AzureVMNames) {
    ...
    $OptionalParameters['subnetName'] = "SIS-WB-SN001-172.21.96.0_21"
    $OptionalParameters['domainToJoin'] = "powerschool.host"
    $OptionalParameters['domainUsername'] = "sundeep.paluru"
    ##Golden Image Value
    $OptionalParameters['templateResourceID'] = '/subscriptions/8e7f0b90-1e44-4729-a2d3-a85c4fdc3625/resourceGroups/SIS-PD-IM-02-T0-RG001/providers/Microsoft.Compute/images/SISPDWB02WIMG001'
    ##Key Vault
    ...
    #$OptionalParameters['pIPAddress'] = Powershell -file ..\..\Scripts\Set-IPAllocator.ps1 -Path ..\..\Scripts\SIS-WB-SN001-172.21.96.0_21.csv -HostName ($AzureVMName + "-Nic")
    #$OptionalParameters['sIPAddress'] = Powershell -file ..\..\Scripts\Set-IPAllocator.ps1 -Path ..\..\Scripts\SIS-WB-SN001-172.21.96.0_21.csv -HostName ($AzureVMName + "-BNic")

    $OptionalParameters['pIPAddress'] = "172.21.96.13"
    $OptionalParameters['sIPAddress'] = "172.21.96.14"
    ...
    if ($ValidateOnly) {
        ...
        New-AzureRmResourceGroupDeployment -Name ((Get-AzureRmContext).Account.Id.Split("@")[0] + '-' + ((Get-Date).ToString('MMdd-HHmmss'))) `
